...
if __name__ == '__main__':
    # ============================================================
    # RRR26 VENUE TEMPLATE — แก้ 3 จุดที่สนามแข่ง (เซียร์รังสิต 30-31 พ.ค. 2569)
    # ============================================================
    # 1) WiFi  : SSID + password ของสนาม (ต้อง 2.4 GHz)
    ...
    #   - cd start_up_robot && python3 config_robot_RRR26.py
    #   - reboot หุ่น (ปิด-เปิดสวิตช์) ให้ config active
    # ============================================================
    robot = MicroROS_Robot(port='/dev/ttyUSB0', debug=False)
    print("Rebooting Device, Please wait.")
    robot.reboot_device()
    robot.set_wifi_config("<SSID>", "<PASSWORD>")          # TODO: SSID + password สนาม (2.4 GHz)
    robot.set_udp_config([0,0,0,0], 8090)            # TODO: ใส่ IP เครื่อง host PC (agent) ของวงนี้
    robot.set_car_type(robot.CAR_TYPE_COMPUTER)
    # robot.set_car_type(robot.CAR_TYPE_RPI5)
    # robot.set_car_type(robot.CAR_TYPE_RISCV)
    robot.set_ros_domain_id(20)                                  # TODO: ให้ตรง ROS_DOMAIN_ID ของ host
